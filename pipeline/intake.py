...
AUDIO_EXT = {".m4a", ".mp3", ".wav", ".aac", ".opus", ".ogg", ".oga", ".flac",
             ".webm", ".mp4"}
TEXT_EXT = {".txt", ".md"}
# The client always converts to one of these before the file reaches the
# inbox (CLAUDE.md §7 — no Pillow, no server-side HEIC decode) — the Shortcut
# and the cockpit's own photo button both do this. HEIC/HEIF are handled
# separately below: recognised-but-unsupported, so a raw HEIC arriving some
# other way (e.g. Syncthing straight from an iPhone) is quarantined with a
# clear next step instead of silently vanishing forever.
...
class Item:
    path: Path
    kind: str                 # "audio" | "text" | "link" | "image" | "image-unsupported"
    captured: datetime
...
def _parse(path: Path, source_hint: str | None = None) -> Item | None:
    ...
    elif ext in UNSUPPORTED_IMAGE_EXT:
        # never silently dropped — process_file() quarantines this kind
        # immediately with a clear "convert on your phone" message, since
        # this server never attempts a HEIC decode (CLAUDE.md §7)
        kind, source = "image-unsupported", "manual"
    else:
        return None
